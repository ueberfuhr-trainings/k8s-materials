...
Erzeuge im Ordner `helm-charts` das Backend-Chart und überführe deine Backend-Manifeste (Deployment mit DB-Umgebungsvariablen, Service, Route) analog zu Übung 8 in die Templates:
...
curl -i https://<backend-route-url>/recipes
```
...
Baue analog ein Chart `recipes-frontend` (Deployment mit `API_BASE_URL`, Service, Route) und definiere darin die Abhängigkeit auf das Backend:
