...
In OpenShift-Clustern sind typischerweise mehrere StorageClasses verfügbar (z.B. für Block- oder File-Storage). Die mit `(default)` markierte wird automatisch verwendet, wenn im PVC keine explizite StorageClass angegeben wird.
...
curl -X POST https://<backend-route-url>/recipes \
...
```bash
curl https://<backend-route-url>/recipes | jq
```
...
```bash
# PVC anzeigen
oc get pvc postgres-data -o yaml
